...
def send_message(message_to_be_sent,to_node):
	...
	if (to_node.strip() == "B"):
		status = proxy_server.rpc_callback_at_nodeB(message_to_be_sent,"A" ,to_node,VECTOR_AT_NODE_A[0])

	elif (to_node.strip() == "C"):
		status = proxy_server.rpc_callback_at_nodeC(message_to_be_sent,"A" ,to_node,VECTOR_AT_NODE_A[0])
...
def recv_message(message,from_node,time_at_node):
	...
	VECTOR_AT_NODE_A[0] += 1
	if(from_node == "B"):
		VECTOR_AT_NODE_A[1] = time_at_node
	
	elif(from_node == "C"):
		VECTOR_AT_NODE_A[2] = time_at_node
	
	print("[*] {",from_node.strip(),"} ---> {A} MSG: ",message,"AFTER Vector Node A: ",VECTOR_AT_NODE_A,"\n")

def rpc_callback_at_nodeA(message_to_be_sent = " ",from_node = " " ,to_node = " ", time_at_node = 0):
